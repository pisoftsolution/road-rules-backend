...
exports.phoneOtpSend = async (req, res) => {
  ...
  try {
    verificationRequest = await twilio.verify
      ...
      .verifications.create({ to: '+' + req.query.phone, channel });
...
exports.phoneOtpVerify = async (req, res) => {
  ...
  try {
    verificationResult = await twilio.verify
      .services(VERIFICATION_SID)
      .verificationChecks.create({ code, to: '+' + req.query.phone });
  } catch (e) {
    console.log(e);
  ...
  if (verificationResult.status === 'approved') {
    ...
    return res
      .status(400)
      .send({
        msg: `Unable To Verify Code. Status: ${verificationResult.status}`
      });
  }
};
